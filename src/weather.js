...
const WEATHER_EFFECTS = {
    STORM: {
        ...
        threatAmplification: { ENV: 1.5, GEO: 1.2 },
    },
    RAIN: {
        movementPenalty: 0.2,
        visibilityModifier: 0.2,
        threatAmplification: { BIO: 1.2 },
    },
    SNOW: {
        movementPenalty: 0.3,
    ...
    DUST_STORM: {
        ...
        threatAmplification: { RAD: 1.3 },
    },
    ACID_RAIN: {
        movementPenalty: 0.1,
        visibilityModifier: 0.1,
        threatAmplification: { BIO: 1.5, RAD: 1.2 },
    },
    RADIOLOGICAL_FALLOUT: {
        movementPenalty: 0,
        visibilityModifier: 0.1,
        threatAmplification: { RAD: 2.0, BIO: 1.3 },
    },
    CLEAR: {
        movementPenalty: 0,
...
export class WeatherSystem {
    ...
    generateWeatherForChunk(chunk, totalEnvSeverity) {
        ...
        if (
            Math.random() < climateVulnerability * 0.1 + envFactor &&
            potentialWeather.length > 1
        ) {
            const adverseTypes = potentialWeather.filter((w) => w !== 'CLEAR');
            weatherType =
                adverseTypes[Math.floor(Math.random() * adverseTypes.length)];
        }

        const effects =
            WEATHER_EFFECTS[weatherType] || WEATHER_EFFECTS['CLEAR'];
     ...
     */
    setWeatherInRegion(region, weatherType, worldState) {
        if (!WEATHER_TYPES.includes(weatherType)) {
            console.error(
                `Invalid weather type specified for control: ${weatherType}`
            );
            return;
        }
        ...
        worldState.voxelWorld.chunks.forEach((chunk) => {
            ...
            const distance = worldState.greatCircleDistance(
                lat,
                lon,
                region.centroid[0],
                region.centroid[1]
