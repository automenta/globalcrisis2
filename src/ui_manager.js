...
export class UIManager {
    ...
    bindEventListeners() {
        ...
        this.togglePlumesButton.addEventListener('click', () => {
            ...
            this.worldState.plumes.forEach(plume => {
        ...
        this.powerModeSelector.addEventListener('change', (event) => {
            if (event.target.name === 'power-mode') {
                this.worldState.voxelWorld.setPowerMode(event.target.value);
            }
        });

        this.deployNetworkButton.addEventListener('click', () => {
            const action = PlayerActions.deploy_network_infrastructure;
            const region = this.selectedThreat ? this.worldState.getRegionForThreat(this.selectedThreat) : null;
            const context = { worldState: this.worldState, playerFaction: this.worldState.playerFaction, selectedThreat: this.selectedThreat, selectedRegion: region };
        ...
        this.launchSatelliteButton.addEventListener('click', () => {
            ...
            const context = { worldState: this.worldState, playerFaction: this.worldState.playerFaction, selectedThreat: null, selectedRegion: null };
            if (this.actionService.executeAction(action, context)) {
        ...
        this.buildResearchOutpostButton.addEventListener('click', () => {
            if (this.selectedThreat) {
                const region = this.worldState.getRegionForThreat(this.selectedThreat);
                if (region && region.owner === 'PLAYER') {
                    if (this.worldState.addBuilding(region, 'RESEARCH_OUTPOST')) {
        ...
        this.buildSensorButton.addEventListener('click', () => {
            if (this.selectedThreat) {
                const region = this.worldState.getRegionForThreat(this.selectedThreat);
                if (region && region.owner === 'PLAYER') {
                    this.worldState.addBuilding(region, 'SENSOR');
                    this.buildPanel.style.display = 'none';
                }
            }
        });
        ...
        this.researchAdvancedAgentsButton.addEventListener('click', () => {
            ...
            if (this.worldState.playerFaction.canAfford(cost)) {
                this.worldState.playerFaction.spend(cost);
                this.worldState.research.advancedAgents = true;
        ...
        this.researchMoonProgramButton.addEventListener('click', () => {
            if (this.worldState.startResearchProject('moon_program')) {
        ...
        this.researchSingularity1Button.addEventListener('click', () => {
            if (this.worldState.startResearchProject('singularity_1')) {
        ...
        this.researchSingularity2Button.addEventListener('click', () => {
            if (this.worldState.startResearchProject('singularity_2')) {
        ...
        this.researchSingularity3Button.addEventListener('click', () => {
            if (this.worldState.startResearchProject('singularity_3')) {
        ...
        this.buildAgentButton.addEventListener('click', () => {
            if (this.selectedThreat) {
                const region = this.worldState.getRegionForThreat(this.selectedThreat);
                if (region && region.owner === 'PLAYER' && this.worldState.buildings.some(b => b.region === region && b.type === 'BASE')) {
                    if (this.worldState.addUnit(region, 'AGENT')) {
                        this.buildPanel.style.display = 'none';
                    }
                } else {
        ...
        this.buildVehicleButton.addEventListener('click', () => {
            if (this.selectedThreat) {
                const region = this.worldState.getRegionForThreat(this.selectedThreat);
                if (region && region.owner === 'PLAYER' && this.worldState.buildings.some(b => b.region === region && b.type === 'BASE')) {
                    if (this.worldState.addUnit(region, 'GROUND_VEHICLE')) {
                        this.buildPanel.style.display = 'none';
                    }
                } else {
        ...
        this.buildAircraftButton.addEventListener('click', () => {
            if (this.selectedThreat) {
                const region = this.worldState.getRegionForThreat(this.selectedThreat);
                if (region && region.owner === 'PLAYER' && this.worldState.buildings.some(b => b.region === region && b.type === 'BASE')) {
                    if (this.worldState.addUnit(region, 'AIRCRAFT')) {
                        this.buildPanel.style.display = 'none';
                    }
                } else {
        ...
        this.buildSatelliteUnitButton.addEventListener('click', () => {
            if (this.selectedThreat) {
                const region = this.worldState.getRegionForThreat(this.selectedThreat);
                if (region && region.owner === 'PLAYER' && this.worldState.buildings.some(b => b.region === region && b.type === 'BASE')) {
                    if (this.worldState.addUnit(region, 'SATELLITE')) {
                        this.buildPanel.style.display = 'none';
                    }
                } else {
        ...
        this.buildBaseButton.addEventListener('click', () => {
            if (this.selectedThreat) {
                const region = this.worldState.getRegionForThreat(this.selectedThreat);
                if (region && region.owner === 'PLAYER') {
                    this.worldState.addBuilding(region, 'BASE');
                    this.buildPanel.style.display = 'none';
                }
            }
        });
    ...
    updateAgentPanel() {
        const agents = this.worldState.agents.filter(a => a.factionId === this.worldState.playerFaction.id);
    ...
    updatePlayerPanel() {
        if (this.worldState.playerFaction) {
            const faction = this.worldState.playerFaction;
        ...
        switch (this.worldState.aiAlertLevel) {
    ...
    updateNarrativeLog() {
        const chronicles = this.worldState.narrativeManager.chronicles;
    ...
    updateGoalsPanel() {
        const goalsState = this.worldState.goalManager.getGoalsState();
    ...
    updateWeatherPanel() {
        ...
        const { chunkCoord } = this.worldState.voxelWorld.worldToVoxel(this.selectedThreat.mesh.position.x, this.selectedThreat.mesh.position.y, this.selectedThreat.mesh.position.z);
        const chunk = this.worldState.voxelWorld.getChunk(chunkCoord.x, chunkCoord.y, chunkCoord.z);
    ...
    updateThreatPanel() {
        ...
        const region = this.worldState.getRegionForThreat(threat);
    ...
    updateResearchPanel() {
        ...
        const research = this.worldState.research;
    ...
    showLocationInfo(point) {
        const { chunkCoord, localCoord } = this.worldState.voxelWorld.worldToVoxel(point.x, point.y, point.z);
        const chunk = this.worldState.voxelWorld.getChunk(chunkCoord.x, chunkCoord.y, chunkCoord.z);
        if (chunk) {
            const materialId = chunk.getMaterial(Math.floor(localCoord.x), Math.floor(localCoord.y), Math.floor(localCoord.z));
            const { lat, lon } = this.worldState.voxelWorld.vector3ToLatLon(point);
            const climateData = this.worldState.climateGrid.getDataAt(lat, lon);
