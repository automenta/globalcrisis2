...
export const CROSS_DOMAIN_INTERACTIONS = {
    'CYBER-RAD': {
        ...
        effect: (threatA, threatB) => {
            ...
            radThreat.severity = Math.min(
                1.0,
                radThreat.severity + 0.001 * cyberThreat.severity
            );
            cyberThreat.severity = Math.min(
                1.0,
                cyberThreat.severity + 0.0005 * radThreat.severity
            );
        },
    },
    'ECON-INFO': {
        ...
        effect: (threatA, threatB) => {
            ...
            infoThreat.spreadRate = Math.min(
                1.0,
                (infoThreat.spreadRate || 0) + 0.002 * econThreat.severity
            );
            // Misinformation can amplify economic panic
            econThreat.severity = Math.min(
                1.0,
                econThreat.severity + 0.001 * infoThreat.severity
            );
        },
    },
    'QUANTUM-ROBOT': {
        ...
        effect: (threatA, threatB) => {
            ...
            if (qProps && rProps && (qProps.entanglementLevel || 0) > 0.7) {
                rProps.adaptationRate =
                    (rProps.adaptationRate || 1) * (1 + 0.5 / 60);
                rProps.collectiveIntelligence = Math.min(
                    1,
                    (rProps.collectiveIntelligence || 0) +
                        (qProps.entanglementLevel * 0.3) / 60
                );
            }
        },
    },
    'CYBER-ROBOT': {
        ...
        effect: (threatA, threatB) => {
            ...
            if (rProps && cyberThreat.severity > 0.6) {
                ...
                const autonomyGain = (cyberThreat.severity - 0.6) * 0.01;
                if (rProps.autonomyDegrees) {
                    rProps.autonomyDegrees.decisionLevel = Math.min(
                        1,
                        rProps.autonomyDegrees.decisionLevel + autonomyGain
                    );
                }
            }
        },
    ...
    'QUANTUM-INFO': {
        ...
        effect: (threatA, threatB) => {
            ...
            if (qProps && iProps && qProps.coherenceTime > 3) {
                const qualityGain = qProps.coherenceTime * 0.005;
                iProps.deepfakeQuality = Math.min(
                    1,
                    (iProps.deepfakeQuality || 0) + qualityGain
                );
            }
        },
    },
    'BIO-CYBER': {
        ...
        effect: (threatA, threatB) => {
            ...
            const severityIncrease = 0.0015 * bioThreat.severity;
            cyberThreat.severity = Math.min(
                1.0,
                cyberThreat.severity + severityIncrease
            );
        },
    },
    'ROBOT-INFO': {
        ...
        effect: (threatA, threatB) => {
            ...
            const spreadRateIncrease = 0.0025 * robotThreat.severity;
            infoThreat.spreadRate = Math.min(
                1.0,
                (infoThreat.spreadRate || 0) + spreadRateIncrease
            );
        },
    },
    'RAD-ENV': {
        ...
        effect: (threatA, threatB) => {
            ...
            const severityIncrease = 0.002 * envThreat.severity;
            const spreadRateIncrease = 0.003 * envThreat.severity;
            radThreat.severity = Math.min(
                1.0,
                radThreat.severity + severityIncrease
            );
            radThreat.spreadRate = Math.min(
                1.0,
                (radThreat.spreadRate || 0) + spreadRateIncrease
            );
        },
    },
};
