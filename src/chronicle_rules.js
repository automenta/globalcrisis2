export const ChronicleRules = {
    crisis_averted: {
        title: 'Crisis Averted in {regionName}',
        description:
            'A {domain} threat that emerged in {regionName} has been successfully neutralized by {factionName}.',
        trigger: 'THREAT_MITIGATED',
        condition: (triggeringEvent, eventLog) => {
            const threatId = triggeringEvent.data.threatId;
            const generationEvent = eventLog.find(
                (e) =>
                    e.eventType === 'THREAT_GENERATED' &&
                    e.data.threatId === threatId
            );
            return generationEvent !== undefined;
        },
        getTemplateData: (triggeringEvent, worldState) => {
            ...
            const faction = worldState.factionManager.factions.find(
                (f) => f.id === triggeringEvent.data.factionId
            );
            return {
    ...
    technocrat_plot: {
        ...
        description:
            'A {domain} threat secretly deployed by the Evil Technocrats in {regionName} has been exposed and neutralized.',
        trigger: 'THREAT_MITIGATED',
        condition: (triggeringEvent, eventLog) => {
            const threatId = triggeringEvent.data.threatId;
            const generationEvent = eventLog.find(
            ...
            return (
                generationEvent &&
                generationEvent.data.isFromAI &&
                triggeringEvent.data.factionId === 'mitigators'
            );
        },
        getTemplateData: (triggeringEvent, worldState) => {
            const threat = triggeringEvent.data.threat;
    ...
    agent_captured_narrative: {
        ...
        description:
            'Agent {agentName} was captured during a failed {missionName} mission in {regionName}. Their fate is unknown.',
    ...
    critical_success: {
        ...
        description:
            'Agent {agentName} achieved a critical success during a high-stakes {missionName} mission in {regionName}, yielding exceptional results.',
        trigger: 'MISSION_SUCCESS',
        condition: (triggeringEvent, worldState) => {
            const agent = worldState.worldState.agents.find(
                (a) => a.name === triggeringEvent.data.agentName
            );
            return (
                agent &&
                agent.level >= 3 &&
                agent.mission &&
                agent.mission.risk > 0.5
            );
        },
        getTemplateData: (triggeringEvent) => ({
            missionName: triggeringEvent.data.missionName,
    ...
    ai_expansion: {
        ...
        description:
            'The Technocrats have expanded their influence by claiming the region of {regionName}.',
        trigger: 'REGION_CLAIMED',
        condition: (triggeringEvent) =>
            triggeringEvent.data.faction === 'Evil Technocrats',
        getTemplateData: (triggeringEvent) => ({
            regionName: triggeringEvent.data.region,
        }),
    },
    satellite_launch_success: {
        ...
        description:
            'The {factionName} have successfully launched a new satellite, boosting their global intelligence capabilities.',
