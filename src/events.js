...
const WorldEvents = [
    {
        ...
        description:
            'A wave of innovation and investment leads to a global economic boom. All factions receive a significant funding boost.',
        weight: 10,
        ...
        effect: (worldState) => {
            ...
            worldState.globalMetrics.economy = Math.min(
                1.0,
                worldState.globalMetrics.economy + 0.1
            );
            worldState.narrativeManager.logEvent('GLOBAL_EVENT', {
                title: 'Economic Boom',
                description: 'The world economy is flourishing.',
            });
        },
    },
    {
        ...
        description:
            'A speculative bubble bursts, causing a slight market correction. Global economy dips slightly.',
        weight: 5,
        ...
        effect: (worldState) => {
            ...
            worldState.narrativeManager.logEvent('GLOBAL_EVENT', {
                ...
                description: 'The global markets have cooled down.',
            });
        },
    },
    {
        ...
        description:
            'A major political scandal erupts in a random region, causing instability and reducing trust in the ruling faction.',
        weight: 10,
        condition: (worldState) => worldState.globalMetrics.trust > 0.3,
        effect: (worldState) => {
            const region =
                worldState.regions[
                    Math.floor(Math.random() * worldState.regions.length)
                ];
            region.stability = Math.max(0, region.stability - 0.2);
            worldState.globalMetrics.trust = Math.max(
                0,
                worldState.globalMetrics.trust - 0.1
            );
            worldState.narrativeManager.logEvent('GLOBAL_EVENT', {
                title: 'Political Scandal',
                description: `A scandal in ${region.name} has rocked the world.`,
            });
        },
    },
    {
        ...
        description:
            "A major scientific breakthrough provides a significant boost to all factions' tech resources.",
        weight: 10,
        ...
        effect: (worldState) => {
            ...
            worldState.narrativeManager.logEvent('GLOBAL_EVENT', {
                title: 'Scientific Breakthrough',
                description: 'A new discovery promises to change the world.',
            });
        },
    },
    {
        ...
        description:
            'A severe natural disaster strikes a random region, causing widespread devastation.',
        weight: 5,
        condition: (worldState) =>
            worldState.regions.some(
                (r) => r.attributes.climateVulnerability > 0.6
            ),
        effect: (worldState) => {
            const vulnerableRegions = worldState.regions.filter(
                (r) => r.attributes.climateVulnerability > 0.6
            );
            const region =
                vulnerableRegions[
                    Math.floor(Math.random() * vulnerableRegions.length)
                ];
            region.economy = Math.max(0, region.economy - 0.3);
            region.stability = Math.max(0, region.stability - 0.3);
            worldState.narrativeManager.logEvent('GLOBAL_EVENT', {
                title: 'Natural Disaster',
                description: `A disaster has struck ${region.name}.`,
            });
        },
    },
    {
        ...
        description:
            'A major solar flare disrupts satellite communications, temporarily disabling all satellites.',
        weight: 3,
        ...
        effect: (worldState) => {
            ...
            worldState.narrativeManager.logEvent('GLOBAL_EVENT', {
                title: 'Solar Flare',
                description: 'Satellite communications have been disrupted.',
            });
        },
    },
];
...
export class EventManager {
    ...
    triggerRandomEvent() {
        ...
        const possibleEvents = WorldEvents.filter((event) =>
            event.condition(this.worldState)
        );
